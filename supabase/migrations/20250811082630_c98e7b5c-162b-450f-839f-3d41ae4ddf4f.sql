-- Fix: Use correct column names in pg_policies (policyname, schemaname, tablename)
...
-- 2) Triggers (create if not exists pattern)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_canais_updated_at'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_subcanais_updated_at'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE policyname = 'Authenticated users can view canais' 
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Authenticated users can view subcanais' 
      AND schemaname = 'public' 
      AND tablename = 'subcanais'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Admins can manage canais' 
      AND schemaname = 'public' 
      AND tablename = 'canais'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE policyname = 'Admins can manage subcanais' 
      AND schemaname = 'public' 
      AND tablename = 'subcanais'
  ) THEN
